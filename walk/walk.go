...
func Walk(t r.Type, fn Handler) error {
	...
	case r.SimpleExpression:
		return walkSimpleExpression(&t, fn)
	case *r.SimpleExpression:
		return walkSimpleExpression(t, fn)
...
func walkIndexOrCallExpression(t *r.IndexOrCallExpression, fn Handler) error {
	if t.SimpleExpression != nil {
		return fn.Handle(t.SimpleExpression)
	} else if t.IndexOrCallExpression != nil {
		if err := fn.Handle(t.IndexOrCallExpression); err != nil {
			return err
		}

		if t.Index != nil {
			return fn.Handle(t.Index)
		} else if t.Call != nil {
...
func walkParenthesizedExpression(t *r.ParenthesizedExpression, fn Handler) error {
	return fn.Handle(&t.Expression)
}
...
func walkSequenceExpression(t *r.SequenceExpression, fn Handler) error {
...
func walkSimpleExpression(t *r.SimpleExpression, fn Handler) error {
	if t.ParenthesizedExpression != nil {
		return fn.Handle(t.ParenthesizedExpression)
	} else if t.CompoundExpression != nil {
		return fn.Handle(t.CompoundExpression)
	}
